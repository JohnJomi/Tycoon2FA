...
Layer 1 behind `run_layers` is real; Layers 2-4 are not implemented and
report themselves as incomplete, so the composite is scored over Layer 1
alone and the report says so. Implementing them requires no change here.
...
def _format_report(
    ...
    lines.append(
        "\nNOTE: Layer 1 is implemented; Layers 2-4 are not, and report as "
        "incomplete rather than clean, so this score reflects header and "
        "domain signals only."
    )
    return "\n".join(lines)
